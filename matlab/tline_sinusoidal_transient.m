...
ZL    = 141.12 - 1j*102.55;      % *complex* load impedance  (Ohm)
L     = 1.0;              % line length (m)
...
x      = linspace(0, L, Nx);
d_lam  = (L - x) / lam;                 % distance from LOAD in wavelengths
L_lam  = L / lam;                       % line length in wavelengths
...
% ---- Voltage subplot
axV = subplot(2,1,1);
hV      = plot(d_lam, zeros(size(x)),'b', 'LineWidth',2); hold on;
hVenvP  = plot(d_lam,  abs(Vss_ph), 'b:',  'LineWidth',1);   % steady-state envelope
hVenvN  = plot(d_lam, -abs(Vss_ph), 'b:',  'LineWidth',1);
yline(0,'k:');
Vlim = 1.25 * max([abs(Vp0), max(abs(Vss_ph))]) * 2;
ylim([-Vlim  Vlim]);  xlim([0 L_lam]);
set(axV, 'XDir', 'reverse');           % zero at load (right), grows leftward
xlabel('distance from load   (L - x) / \lambda');
...
% ---- Current subplot
axI = subplot(2,1,2);
hI      = plot(d_lam, zeros(size(x)),'r', 'LineWidth',2); hold on;
hIenvP  = plot(d_lam,  abs(Iss_ph), 'r:',  'LineWidth',1);
hIenvN  = plot(d_lam, -abs(Iss_ph), 'r:',  'LineWidth',1);
yline(0,'k:');
Ilim = 1.25 * max([abs(Vp0)/Z0, max(abs(Iss_ph))]) * 2;
ylim([-Ilim  Ilim]);  xlim([0 L_lam]);
set(axI, 'XDir', 'reverse');           % zero at load (right), grows leftward
xlabel('distance from load   (L - x) / \lambda');
...
% source / load markers
for ax = [axV axI]
    plot(ax, L_lam, 0, 'ks','MarkerFaceColor',[.85 .85 .85],'MarkerSize',10);  % source at left (d = L/lam)
    plot(ax, 0,     0, 'ko','MarkerFaceColor',[.85 .85 .85],'MarkerSize',10);  % load at right (d = 0)
end
...
subplot(2,1,1);
plot(d_lam, abs(Vss_ph),'b','LineWidth',2); hold on;
plot(d_lam, real(Vss_ph),'b--'); plot(d_lam, imag(Vss_ph),'b:');
grid on; xlabel('(L - x) / \lambda'); ylabel('V phasor');
set(gca,'XDir','reverse'); xlim([0 L_lam]);
legend('|V_{ss}(x)|','Re\{V_{ss}\}','Im\{V_{ss}\}','Location','best');
title('Analytical steady-state voltage phasor V_{ss}(x)');

subplot(2,1,2);
plot(d_lam, abs(Iss_ph),'r','LineWidth',2); hold on;
plot(d_lam, real(Iss_ph),'r--'); plot(d_lam, imag(Iss_ph),'r:');
grid on; xlabel('(L - x) / \lambda'); ylabel('I phasor');
set(gca,'XDir','reverse'); xlim([0 L_lam]);
